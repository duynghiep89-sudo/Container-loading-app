...
# --- [BẢO TỒN] CSS CHỈ IN PHẦN KẾT QUẢ ---
st.markdown("""
    <style>
    @media print {
...
# --- [BẢO TỒN] HÀM VẼ 3D VỚI GÓC NHÌN ƯU TIÊN GÓC DƯỚI & VIỀN ĐẬM ---
def draw_3d_loading(bin_obj, sku_colors, sku_counts, container_type):
    ...
    for item in bin_obj.items:
        ...
        # [BẢO TỒN] Đường viền sắc nét Width=3
    ...
    # [BẢO TỒN] Layout Camera ưu tiên góc dưới & Height=800
    fig.update_layout(
        scene=dict(
...
with st.sidebar:
    ...
    # [BẢO TỒN] Trừ 2cm biên và 3cm trần an toàn
...
with tab_calc:
    ...
    # Lấy danh sách SKU từ thư viện làm gợi ý
    ...
    manual_entry = st.data_editor(
        ...
        num_rows="dynamic",
        column_config={
            # [NÂNG CẤP] Selectbox cho SKU nhưng vẫn cho phép nhập mới
            "SKU": st.column_config.SelectboxColumn("Chọn/Nhập SKU", options=available_skus, required=True),
            "Quantity": st.column_config.NumberColumn("Số lượng", default=1)
        },
        key="calc_editor"
    )
    
    # [NÂNG CẤP] Tự động điền kích thước từ thư viện
    ...
    if st.button("🚀 BẮT ĐẦU TÍNH TOÁN"):
        ...
        if not calc_df.empty:
            # [BẢO TỒN] Tính CBM chuẩn đơn vị mét
            ...
            with st.spinner('🛠️ Đang xử lý...'):
                ...
                # [BẢO TỒN] Hiển thị thông số lên trên
                ...
                # [BẢO TỒN] Nút in thông minh ẩn Sidebar/Tabs
